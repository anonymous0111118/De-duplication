...
Our **manual analysis** of this example is as follows: In C language, the char type is regarded as a signed type by default. When the char type is compared with the unsigned char type, the unsigned char is converted to char by default, so the value of b It should be -1, but when it is defined with const, b is not staged under the optimization level of O2, but is compared with c as an integer of size 255, and an incorrect output is obtained.
